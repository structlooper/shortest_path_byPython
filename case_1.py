...
def minimum_path(matrix,m,n):
    ...
    for i in range(1,n+1):
        ...
        # if pawn at that path in that case it finds another shortest path........
        if pawn:
            while n < len(pawn):
                ver_path = mini_path[i-n][j]
                hor_path = mini_path[i][j-n]
                cross_path = mini_path[i-n][j-n]
            
                another_path = matrix[i][j] + min(ver_path,hor_path,cross_path) 
            return another_path[m][n]
